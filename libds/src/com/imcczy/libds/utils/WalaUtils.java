...
public class WalaUtils {
    private static final Logger logger = LoggerFactory.getLogger(com.imcczy.libds.utils.WalaUtils.class);
	...
	*/
    ...
    public static boolean isAppClass(IClass clazz) {
        ...
        if (clazz.getClassHierarchy() != null)
            ...
            return !isEmptyInnerClass && !isAndroidResourceClass(clazz);
    }

    public static boolean isUsefullClass(IClass clazz) {
        // filter empty dummy classes
        // possibly related too: http://bugs.java.com/bugdatabase/view_bug.do?bug_id=4295934
        boolean isEmptyInnerClass = WalaUtils.isInnerClass(clazz)
                && isAnonymousInnerClass(clazz)
    ...
    public static String getFullClassName(IClass clazz){
        String clazzName = clazz.getName().toString().substring(1).replace("/",".");
        return clazzName.endsWith(";") ? clazzName.substring(0, clazzName.length() - 1) : clazzName;
    }
    ...
    public static boolean isAndroidResourceClass(IClass clazz) {
        ...
        return clazzName.equals("R") || clazzName.startsWith("R$") || clazzName.equals("BuildConfig")
                || clazzName.equals("Manifest") || clazzName.startsWith("Manifest$");
     ...
     * added by imcczy
     */
    public static boolean isAndroidClass(IClass clazz) {
        String clazzName = simpleName(clazz);
        return clazzName.startsWith("android.support");
    }

    /*
        public static boolean isApkClass(IClass clazz){
            String clazzName = simpleName(clazz);
            if ( ApkHandler.Packagename.packageName!= null)
                return clazzName.startsWith(ApkHandler.Packagename.packageName);
            return false;
        }
    */
