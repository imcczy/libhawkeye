...
public class  ClassHierarchy implements IClassHierarchy{
  ...
  ClassHierarchy(AnalysisScope scope, ClassLoaderFactory factory, Language language, IProgressMonitor progressMonitor, Map<TypeReference, Node> map)
    ...
    this(scope, factory, Collections.singleton(language), progressMonitor, map,null);
  }

  ClassHierarchy(AnalysisScope scope, ClassLoaderFactory factory, IProgressMonitor progressMonitor, Map<TypeReference, Node> map,IClassLoader iClassLoader)
      throws ClassHierarchyException, IllegalArgumentException {
    this(scope, factory, scope.getLanguages(), progressMonitor, map,iClassLoader);
  }

  ClassHierarchy(AnalysisScope scope, ClassLoaderFactory factory, Collection<Language> languages,
      IProgressMonitor progressMonitor, Map<TypeReference, Node> map,IClassLoader iClassLoader) throws ClassHierarchyException, IllegalArgumentException {
    ...
    try {
      ...
      for (ClassLoaderReference ref : scope.getLoaders()) {
        ...
        if (langNames.contains(ref.getLanguage())) {
            IClassLoader icl;
            if (ref.equals(ClassLoaderReference.Primordial)) {
                if (iClassLoader != null){
                    loaders[idx++] = iClassLoader;
                }
                else {
                    loaders[idx++] = factory.getLoader(ref, this, scope);
                }
            }else {
      ...
      for (IClassLoader icl : loaders) {
        ...
        if (icl.getReference().equals(ClassLoaderReference.Primordial)){
            if (map.size() == 0)
  ...
  private void addAllClasses(IClassLoader loader, IProgressMonitor progressMonitor) throws CancelCHAConstructionException {
    ...
    */
    for (IClass iClass: loader.getAllClasses()){
        if (progressMonitor != null) {
            if (progressMonitor.isCanceled()) {
                throw new CancelCHAConstructionException();
            }
        }
        boolean added = addClass(iClass);
        if (!added) {
            toRemove.add(iClass);
        }
    }
   ...
   */
  @Override
  public void addApkClass() throws ClassHierarchyException {
      try {
          for (ClassLoaderReference ref : scope.getLoaders()) {
              if(ref.getName().toString().equals("Application")){
                  IClassLoader icl = factory.getApkLoader(ref, this, scope);
